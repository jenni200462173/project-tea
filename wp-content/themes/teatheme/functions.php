...
function tea_review(){
	$labels = array(
		'name' 			=> ('Teas'),
		'singular_name' => ('Tea'),
		'menu_name'		=> ('Tea Reviews')
	);
	$args = array(
		'label'			=> ('teas'),
		'description'	=> ('tea Reviews'),
		'labels'		=> $labels,
		'supports'		=> array(
			'title',
			'editor',
	...
	register_post_type('tea_review',$args);

}
add_action('init', 'tea_review',0);

// add a shortcode for our tea reviews 
function tea_shortcode(){
	...
	if($query->have_posts()) : 
		while($query->haves_posts()) :
			$query->the_post();
			$result .= '<div>';
			$result .='<span class"tea-image">' . get_the_post_thumgnail() . '</span>';
...
add_shortcode('teas','tea_shortcode');
